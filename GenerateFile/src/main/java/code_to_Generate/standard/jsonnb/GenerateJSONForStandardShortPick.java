...
public class GenerateJSONForStandardShortPick {
	...
	public static final String LOCAL_CSV_FOLDER_PATH = "C:/app/tmp/";
	...
	public static void main(String[] args) {
		...
		IntStream.range(0, NUMBER_OF_FILES).parallel().forEach(iCount -> {
			String strFileName = "STANDARDSHORTPICK" + iCount + ".txt";
			String strJson = generateJSONPick(NUMBER_OF_PICS);
			executor.submit(new CreateFile(strFileName, strJson, LOCAL_CSV_FOLDER_PATH));
			try {
				Thread.sleep(500);
	...
	private static String generateJSONPick(int iPicks) {
		...
		for (int i = 0; i < iPicks; i++) {
			...
			oPickObj.put("pickedQuantity", 3);
			oPickObj.put("userID", "6RS");
			oPickObj.put("deviceID", "chuck");
